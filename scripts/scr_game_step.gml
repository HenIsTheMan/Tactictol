...
if(time1 != 0 && time2 != 0 && device_mouse_check_button_pressed(0,mb_left) && point_in_rectangle(device_mouse_x(0),device_mouse_y(0),0,64,160,64+160)){
    if(grid[# x/32,y/32 - 2] == -5 && (placed == 0 || block)){
        ...
        for(m = 0;m < 3;m++){
            for(n = 0;n < 3;n++){
                sum1 = ds_grid_get(grid,m,n) + ds_grid_get(grid,m+1,n+1) + ds_grid_get(grid,m+2,n+2); //sum1
                sum2 = ds_grid_get(grid,m+2,n) + ds_grid_get(grid,m+1,n+1) + ds_grid_get(grid,m,n+2); //sum2
        ...
        if(ds_grid_get_sum(gridCheck,0,0,5,5) = 3){
...
if(keyboard_check_pressed(vk_space)){
    ...
    ds_grid_clear(gridCheck,0);
    turn++;
    virtual_key_delete(key);
    key = virtual_key_add(128*(turn & 1),256*(~turn & 1),32,32,vk_space);
